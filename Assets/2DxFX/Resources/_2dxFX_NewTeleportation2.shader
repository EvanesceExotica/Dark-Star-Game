...
fixed4 txt1 =  tex2D(_MainTex, uv);
float talpha = txt1.a;

float step2 = smoothstep(0.3, 1, _Fade);
fixed4 txt2 = tex2D(_MainTex2, uv - float2(0, step2*0.4));
fixed4 txt2r = tex2D(_MainTex2, uv+float2(txt2.r*0.3,txt2.r*0.2));
fixed4 rtxt = txt2.r;
rtxt.a = talpha;
...
fixed4 txt3 = tex2D(_MainTex2, uv - float2(txt2.r*0.05*_Distortion+uv.x, _Fade))*8;
btwa += txt3.g*2;
